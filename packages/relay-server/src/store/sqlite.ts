...
// ---------------------------------------------------------------------------
// Nostr pubkey (hex) → session ID lookup — rebuilt from DB on load
// ---------------------------------------------------------------------------
...
function rowToSession(row: SessionRow): Session {
  ...
  // Load Nostr pubkeys and rebuild the module-level pubkeyIndex
  const pubkeyRows = stmts.getPubkeysForSession.all({ $session_id: row.id }) as PubkeyRow[];
  const nostrPubkeys = new Map<string, string>();
  for (const pr of pubkeyRows) {
    nostrPubkeys.set(pr.pubkey, pr.token);
    pubkeyIndex.set(pr.pubkey, row.id);
  }
...
export function sweepExpiredSessions(): number {
  ...
  for (const { id } of expired) {
    // Clean up pubkeyIndex for expired sessions
    const pubkeyRows = stmts.getPubkeysForSession.all({ $session_id: id }) as PubkeyRow[];
    for (const pr of pubkeyRows) {
      pubkeyIndex.delete(pr.pubkey);
    }
