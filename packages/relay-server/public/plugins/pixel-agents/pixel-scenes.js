...
window.PixelSceneConfig = (() => {
  ...
  // ── Helper: add standard wall border (rows 0-1 top, row 13 bottom, col 0 + 19 sides) ──
  function addWalls(map) {
    for (let x = 0; x < MAP_W; x++) {
      map[0][x] = T.WALL;
      map[1][x] = T.WALL;
      map[MAP_H - 1][x] = T.WALL; // bottom wall prevents sprites clipping off-canvas
    }
  ...
  const garden = (() => {
    ...
    // Bottom hedge wall (prevents sprites clipping off-canvas)
    fillRect(map, 0, MAP_H - 1, MAP_W - 1, MAP_H - 1, T.WALL);
  ...
  const waterfront = (() => {
    ...
    // Dock floor (left 2/3)
    fillRect(map, 1, 2, 13, 12, T.FLOOR);
    // Water (right 1/3 — the harbor)
    fillRect(map, 14, 2, 19, 13, T.WATER);
    // Bottom wall (prevents sprites clipping off-canvas — set after water so left dock gets wall)
    fillRect(map, 0, MAP_H - 1, 13, MAP_H - 1, T.WALL);
  ...
  const winterLodge = (() => {
    ...
    fillRect(map, 1, 12, 18, 12, T.SNOW); // row 13 stays as wall
  ...
  const harvestField = (() => {
    const map = makeMap(T.GRASS);
    // Fence walls top, bottom, and sides
    fillRect(map, 0, 0, 19, 1, T.WALL);
    fillRect(map, 0, MAP_H - 1, MAP_W - 1, MAP_H - 1, T.WALL); // bottom fence
    for (let y = 0; y < MAP_H; y++) {
      map[y][0] = T.WALL;
      map[y][MAP_W - 1] = T.WALL;
    }
  ...
  const cliffOverlook = (() => {
    ...
    fillRect(map, 0, 0, 19, 1, T.WALL);
    // Bottom wall (prevents sprites clipping off-canvas)
    fillRect(map, 0, MAP_H - 1, MAP_W - 1, MAP_H - 1, T.WALL);
