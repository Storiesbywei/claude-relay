...
window.PixelCreatureConfig = (() => {
  ...
  const specialCreatures = {
    ...
    pinkPanda: {
      ...
      trigger: { type: 'agentProximity', minAgents: 2, proximityRadius: 3, durationMs: 60000 },
      speed: 1.0,
      scaleMultiplier: 1.0,
      behavior: 'mediator',
    ...
    rubberDuck: {
      name: 'Rubber Duck',
      spriteFile: 'special-5-v1.png',
      trigger: { type: 'random', probability: 0.003 },
      speed: 0.8,
      scaleMultiplier: 1.0,
      behavior: 'wander',
      despawnCondition: { type: 'timer', durationMs: 25000 },

      /**
       * Gentle yellow glow and floating bubble particles for the rubber duck.
       * A nod to rubber-duck debugging.
       * @param {CanvasRenderingContext2D} ctx
       * @param {number} SCALE
       * @param {number} x
       * @param {number} y
      ...
      drawEffect(ctx, SCALE, x, y, timestamp) {
        ...
        const pulse = 0.15 + 0.08 * Math.sin(t * 2);
        const glow = ctx.createRadialGradient(x, y, 2 * SCALE, x, y, glowR);
        ...
        // Floating thought bubbles (rubber duck debugging)
        for (let i = 0; i < 3; i++) {
          const phase = t * 0.6 + i * 2.1;
          const life = (phase % 3) / 3;
          const bx = x + Math.sin(phase * 1.5) * 4 * SCALE;
          const by = y - life * 14 * SCALE - 6 * SCALE;
          const size = (1 - life * 0.6) * 2 * SCALE;
    ...
    bunny: {
      ...
      scaleMultiplier: 0.8,
      behavior: 'tease',
      despawnCondition: { type: 'timer', durationMs: 8000 },
      rewardTiers: [
  ...
  const gatoPersonalities = {
    ...
    mischievous: {
      spriteFile: 'gato-v2.png',
      personality: 'mischievous',
      fleeFromKnights: true,
      fleeRadius: 3,
      behavior: 'knockThingsOff',
      targetFurniture: ['plant', 'desk'],
      napDuration: { min: 4000, max: 10000 },
      trigger: { minTension: 0.4 },
      wanderTargets: ['desk', 'plant'],
    },

    sentinel: {
      spriteFile: 'gato-3-v2.png',
      personality: 'sentinel',
      fleeFromKnights: false,
      fleeRadius: 0,
      behavior: 'guardDesk',
      guardRadius: 2,
      napDuration: { min: 10000, max: 25000 },
      trigger: { activeObstacleCount: 3 },
      wanderTargets: null,
    },
  };
  ...
  const DAILY_SPECIALS = {
    ...
    3: 'rubberDuck',    // Wednesday — rubber duck debugging hump day
  ...
  function checkSpecialSpawns(context) {
    ...
    if (canSpawn('pinkPanda') && agents && agents.size >= 2) {
      ...
      for (let i = 0; i < agentList.length; i++) {
        for (let j = i + 1; j < agentList.length; j++) {
          ...
          const radius = specialCreatures.pinkPanda.trigger.proximityRadius || 3;
          if (dist <= radius && a.proximityStart && b.proximityStart) {
    ...
    // Rubber Duck — random rare spawn (rubber duck debugging companion)
    if (canSpawn('rubberDuck') && Math.random() < specialCreatures.rubberDuck.trigger.probability) {
      spawns.push('rubberDuck');
    }
    ...
    if (dailyKey && canSpawn(dailyKey) && !spawns.includes(dailyKey)) {
      ...
      } else if (dailyKey === 'rubberDuck') {
        // Higher spawn probability on featured day (3x normal)
        if (Math.random() < specialCreatures.rubberDuck.trigger.probability * 3) {
          spawns.push('rubberDuck');
        }
      }
  ...
  function selectGatoPersonality(context) {
    ...
    // Mischievous cat — appears in tense environments
    if (tension >= (gatoPersonalities.mischievous.trigger.minTension || 0.4)) {
      candidates.push({ key: 'mischievous', weight: 12 });
    }

    // Sentinel cat — appears when there are many obstacles/tasks
    if (context.activeObstacleCount >= (gatoPersonalities.sentinel.trigger.activeObstacleCount || 3)) {
      candidates.push({ key: 'sentinel', weight: 15 });
    }
