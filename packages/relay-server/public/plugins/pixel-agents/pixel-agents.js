...
const PixelAgents = (() => {
  ...
  const SHEET = {
    ...
    walkDown:  [0, 0, 6],
    walkUp:    [1, 0, 6],  // Row 1: walk-up animation (facing away)
    idleDown:  [0, 0, 2],  // first 2 frames of walk = idle bob
    idleUp:    [1, 0, 2],  // first 2 frames of walk-up = idle facing away
    walkSide:  [2, 0, 8],
    idleSide:  [2, 0, 2],
  ...
  // Core pool — always loaded, used for agent assignment
  const CHAR_SPRITES = [
    'main-character-v1.png',
    'character-1-v1.png',
    'character-2-v1.png',
    'character-3-v1.png',
    'character-4-v1.png',
    ...
    'character-20-v1.png',
    'character-21-v1.png',
    'basic-character-v1.png',
  ];

  // v2 variants — loaded lazily, used as alternate skins
  const EXTRA_CHAR_SPRITES = [
    'main-character-v2.png',
    'character-1-v2.png',
    'character-2-v2.png',
    'character-3-v2.png',
    'character-4-v2.png',
    ...
    'character-20-v2.png',
    'character-21-v2.png',
    'basic-character-v2.png',
  ];

  // Season-themed sprites — merged into active pool based on current season
  const SEASON_SPRITES = {
    spring: ['spring-1-v1.png', 'spring-2-v1.png', 'spring-3-v1.png', 'spring-4-v1.png'],
    summer: ['summer-1-v1.png', 'summer-2-v1.png', 'summer-3-v1.png', 'summer-4-v1.png'],
    autumn: ['autumn-1-v1.png', 'autumn-2-v1.png', 'autumn-3-v1.png', 'autumn-4-v1.png'],
    winter: ['winter-1-v1.png', 'winter-2-v1.png', 'winter-3-v1.png', 'winter-4-v1.png'],
  };

  // Season v2 variants — loaded lazily alongside season sprites
  const EXTRA_SEASON_SPRITES = {
    spring: ['spring-1-v2.png', 'spring-2-v2.png', 'spring-3-v2.png', 'spring-4-v2.png'],
    summer: ['summer-1-v2.png', 'summer-2-v2.png', 'summer-3-v2.png', 'summer-4-v2.png'],
    autumn: ['autumn-1-v2.png', 'autumn-2-v2.png', 'autumn-3-v2.png', 'autumn-4-v2.png'],
    winter: ['winter-1-v2.png', 'winter-2-v2.png', 'winter-3-v2.png', 'winter-4-v2.png'],
  };

  /**
   * Build the active sprite pool by merging CHAR_SPRITES with the current
   * season's sprites. Called on init and when the season changes.
   * @returns {string[]} combined sprite file list
   */
  function buildActiveSpritePool() {
    const season = (window.__pixelTimeOverride && window.__pixelTimeOverride.season) || getCurrentSeason();
    const seasonFiles = SEASON_SPRITES[season] || [];
    return [...CHAR_SPRITES, ...seasonFiles];
  }

  // The active pool — rebuilt when season changes
  let activeSpritePool = buildActiveSpritePool();

  // Label colors for name tags (expanded to match larger sprite pool)
  const LABEL_COLORS = [
    '#58a6ff', '#7c6aef', '#f0883e', '#3fb950', '#f85149', '#db61a2',
    '#d2a8ff', '#79c0ff', '#ffa657', '#56d364', '#ff7b72', '#ff9bce',
    '#58a6ff', '#7c6aef', '#f0883e', '#3fb950', '#f85149', '#db61a2',
    '#d2a8ff', '#79c0ff', '#ffa657', '#56d364', '#ff7b72', '#ff9bce',
    '#58a6ff', '#7c6aef', '#f0883e',
  ];
  ...
  const TIME_TINTS = {
    sunrise: { r: 255, g: 220, b: 160, a: 0.08 },
    day:     null, // no tint
    sunset:  { r: 255, g: 190, b: 140, a: 0.10 },
    evening: { r: 210, g: 195, b: 230, a: 0.05 },
    night:   { r: 200, g: 195, b: 225, a: 0.08 },
  };
  ...
  const SEASON_PARTICLES = {
    ...
    winter: { type: 'snow',    colors: ['#f5f0ea', '#ebe4d8', '#ffffff'], count: 2 },
  };
  ...
  // ── Turn-Pause System ──
  // When entities change between vertical/horizontal movement, they briefly
  // pause in idle for a few frames. This masks the "moonwalk" artifact caused
  // by using walkDown sprites for all directions (walkSide row 2 was removed
  // because it looked crouched/horizontal).
  const TURN_PAUSE_MS = 200;  // brief idle pause when changing direction axis

  function isAxisChange(oldFacing, newFacing) {
    // Returns true if direction changed between vertical (up/down) and horizontal (left/right)
    const verticals = new Set(['up', 'down']);
    const horizontals = new Set(['left', 'right']);
    if (!oldFacing || !newFacing || oldFacing === newFacing) return false;
    return (verticals.has(oldFacing) && horizontals.has(newFacing)) ||
           (horizontals.has(oldFacing) && verticals.has(newFacing));
  }
  ...
  const SPECIAL_SPRITES = [
    ...
    'special-4-v1.png', 'special-5-v1.png', 'basic-bunny-character-v1.png',
  ];
  // Special creature v2 variants + bunny v2 — loaded lazily
  const EXTRA_SPECIAL_SPRITES = [
    'special-1-v2.png', 'special-2-v2.png', 'special-3-v2.png',
    'special-4-v2.png', 'special-5-v2.png', 'basic-bunny-character-v2.png',
  ];
  const EXTRA_GATO_SPRITES = [
    'gato-4-v1.png', 'gato-5-v1.png',
    'gato-v2.png', 'gato-2-v2.png', 'gato-3-v2.png', 'gato-4-v2.png', 'gato-5-v2.png',
  ];

  function loadSprites() {
    // Gather all season sprites for the current season
    const seasonFiles = SEASON_SPRITES[getCurrentSeason()] || [];
    let loaded = 0;
    let total = CHAR_SPRITES.length + seasonFiles.length + 1 + SPECIAL_SPRITES.length; // +1 for shadow
    ...
    // Load current season's sprites (blocking — part of active pool)
    ...
    // Load extra character v2 sprites (non-blocking — lazy loaded for variant skins)
    for (const file of EXTRA_CHAR_SPRITES) {
      const img = new Image();
      img.src = `/plugins/pixel-agents/assets/sprites/characters/${file}`;
      spriteImages.set(file, img);
    }

    // Load extra season v2 sprites (non-blocking)
    const extraSeasonFiles = EXTRA_SEASON_SPRITES[getCurrentSeason()] || [];
    for (const file of extraSeasonFiles) {
      const img = new Image();
      img.src = `/plugins/pixel-agents/assets/sprites/characters/${file}`;
      spriteImages.set(file, img);
    }

    // Load all other seasons' sprites lazily (non-blocking — ready for season change)
    for (const [season, files] of Object.entries(SEASON_SPRITES)) {
      if (season === getCurrentSeason()) continue; // already loaded above
    ...
    // Load extra special creature v2 sprites (non-blocking)
    for (const file of EXTRA_SPECIAL_SPRITES) {
      const img = new Image();
      img.src = `/plugins/pixel-agents/assets/sprites/characters/${file}`;
      spriteImages.set(file, img);
    }
  ...
  function buildDefaultWorkshop() {
    ...
    // Walls (top 2 rows, bottom row, left + right columns)
    for (let x = 0; x < MAP_W; x++) {
      tilemap[0][x] = TILE.WALL;
      tilemap[1][x] = TILE.WALL;
      tilemap[MAP_H - 1][x] = TILE.WALL; // bottom wall prevents sprites clipping off-canvas
    }
  ...
  // ── BFS Pathfinding ──
  // Walkable bounds: exclude outermost border tiles so 24px sprites
  // (which are larger than the 16px tile grid) never render off-canvas.
  const WALK_MIN_X = 1;
  const WALK_MIN_Y = 1;
  const WALK_MAX_X = MAP_W - 2;  // 18
  const WALK_MAX_Y = MAP_H - 2;  // 12
  ...
  function findPath(sx, sy, ex, ey) {
    ...
    while (queue.length > 0) {
      ...
      for (const { dx, dy } of [{ dx: 0, dy: -1 }, { dx: 0, dy: 1 }, { dx: -1, dy: 0 }, { dx: 1, dy: 0 }]) {
        ...
        const ny = y + dy;
        const key = `${nx},${ny}`;
        if (nx < WALK_MIN_X || nx > WALK_MAX_X || ny < WALK_MIN_Y || ny > WALK_MAX_Y) continue;
    ...
    for (const item of furniture) {
      ...
      switch (item.type) {
        ...
        case 'monitor': {
          ...
          if (active) {
            ctx.fillStyle = 'rgba(74, 154, 255, 0.25)';
  ...
  function getAnimDef(agent) {
    // Pick animation row/frames based on agent state and facing direction
    switch (agent.state) {
      case 'walking': {
        if (agent.facing === 'up') return SHEET.walkUp;
      ...
      case 'thinking':
        return agent.facing === 'up' ? SHEET.idleUp : SHEET.idleDown;
      case 'waiting':
        return agent.facing === 'up' ? SHEET.idleUp : SHEET.idleDown;
      case 'idle':
      default:
  ...
  function shouldFlip(agent) {
    // Sprite sheet walkSide faces LEFT by default
    // Flip when walking RIGHT to mirror the left-facing sprite
    if (agent.facing === 'right') return true;
    if (agent.facing === 'left') return false;
    return false;
  }

  function drawAgent(agent, time) {
    const spriteFile = activeSpritePool[agent.charIdx % activeSpritePool.length];
    ...
    ctx.fillStyle = 'rgba(62, 46, 34, 0.75)';
    ...
    // Status dot
    const statusColors = {
      idle: '#a89880', walking: '#3fb950', typing: '#f0883e',
      reading: '#58a6ff', thinking: '#7c6aef', waiting: '#a89880',
    };
    ctx.fillStyle = statusColors[agent.state] || '#a89880';
    ctx.beginPath();
  ...
  function drawStateOverlay(agent, time) {
    ...
    if (agent.chatBubbleTimer > 0) {
      ...
      ctx.fillStyle = `rgba(92, 74, 58, ${0.7 * bubbleAlpha})`;
      for (let i = 0; i < 3; i++) {
  ...
  function updateAgent(agent, dt) {
    // ── Turn-pause: hold idle briefly when switching between vertical/horizontal ──
    if (agent.turnPauseTimer > 0) {
      agent.turnPauseTimer -= dt;
      // Show idle animation in the NEW facing direction during the pause
      agent.state = 'idle';
      if (agent.turnPauseTimer <= 0) {
        agent.turnPauseTimer = 0;
        agent.state = 'walking';
      }
      return; // freeze movement during turn pause
    }
    ...
    if (agent.path && agent.path.length > 0) {
      ...
      // Detect axis change and trigger turn pause
      if (isAxisChange(agent.prevFacing, newFacing)) {
        agent.facing = newFacing;
        agent.prevFacing = newFacing;
        agent.turnPauseTimer = TURN_PAUSE_MS;
        agent.state = 'idle';
        return; // start pause, skip this frame's movement
      }
      agent.facing = newFacing;
      agent.prevFacing = newFacing;

      if (dist < speed) {
        // Smooth snap: lerp the final sub-pixel step instead of jumping
        const snapLerp = dist / Math.max(speed, 0.001);
        agent.renderX = agent.renderX + dx * snapLerp;
        agent.renderY = agent.renderY + dy * snapLerp;
        // Then clamp to exact tile position
        ...
        // Update facing for the next segment if path continues
        if (agent.path.length > 0) {
          const next = agent.path[0];
          const ndx = next.x - agent.x;
          const ndy = next.y - agent.y;
          let nextFacing;
          if (Math.abs(ndx) > Math.abs(ndy)) {
            nextFacing = ndx < 0 ? 'left' : 'right';
          } else {
            nextFacing = ndy < 0 ? 'up' : 'down';
          }
          // Trigger turn pause at segment boundary if axis changes
          if (isAxisChange(agent.facing, nextFacing)) {
            agent.facing = nextFacing;
            agent.prevFacing = nextFacing;
            agent.turnPauseTimer = TURN_PAUSE_MS;
            agent.state = 'idle';
            return;
          }
      ...
      // Clamp render position to walkable bounds so sprites stay on-canvas
      agent.renderX = Math.max(WALK_MIN_X, Math.min(WALK_MAX_X, agent.renderX));
      agent.renderY = Math.max(WALK_MIN_Y, Math.min(WALK_MAX_Y, agent.renderY));
  ...
  function gameLoop(timestamp) {
    ...
    if (seasonCheckTimer >= SEASON_CHECK_INTERVAL) {
      ...
      // Rebuild active sprite pool when season changes
      if (prevSeason !== currentSeason) {
        activeSpritePool = buildActiveSpritePool();
      }
    }
    ...
    ctx.fillStyle = 'rgba(62, 46, 34, 0.55)';
  ...
  function spawnAgent(id, name) {
    ...
    const agent = {
      ...
      prevFacing: 'down',    // previous facing for turn-pause detection
      turnPauseTimer: 0,     // ms remaining in turn pause
      frame: 0,
      stateTimer: 0,
      deskIdx,
      path: null,
  ...
  // Dungeon knight variants — alternate skins loaded lazily
  const EXTRA_KNIGHT_SPRITES = [
    'dg-knight-1-v1.png', 'dg-knight-1-v2.png',
    'dg-knight-2-v1.png', 'dg-knight-2-v2.png',
    'dg-knight-3-v1.png', 'dg-knight-3-v2.png',
    'dg-knight-4-v1.png', 'dg-knight-4-v2.png',
  ];
  ...
  function loadKnightSprites() {
    ...
    // Lazy-load dungeon knight variants (non-blocking)
    for (const file of EXTRA_KNIGHT_SPRITES) {
      const img = new Image();
      img.src = `/plugins/pixel-agents/assets/sprites/characters/${file}`;
      knightImages.set(file, img);
    }
  ...
  function updateObstacle(obs, dt) {
    ...
    // Turn-pause for knights
    if (obs.turnPauseTimer > 0) {
      obs.turnPauseTimer -= dt;
      if (obs.turnPauseTimer <= 0) obs.turnPauseTimer = 0;
      return; // freeze movement during turn pause
    }
    ...
    if (obs.path && obs.path.length > 0) {
      ...
      // Detect axis change and trigger turn pause
      if (isAxisChange(obs.prevFacing, newFacing)) {
        obs.facing = newFacing;
        obs.prevFacing = newFacing;
        obs.turnPauseTimer = TURN_PAUSE_MS;
        return;
      }
      obs.facing = newFacing;
      obs.prevFacing = newFacing;
      ...
      obs.renderX = Math.max(WALK_MIN_X, Math.min(WALK_MAX_X, obs.renderX));
      obs.renderY = Math.max(WALK_MIN_Y, Math.min(WALK_MAX_Y, obs.renderY));
      return;
    }
  ...
  function drawObstacle(obs, timestamp) {
    ...
    // Draw knight sprite — direction-aware animation
    if (img && img.complete && img.naturalWidth > 0) {
      // Use persisted facing direction (set during updateObstacle movement)
      const knightFacing = obs.facing || 'down';

      let anim;
      if (obs.state === 'lurking') {
        anim = knightFacing === 'up' ? SHEET.idleUp : SHEET.idleDown;
      } else if (knightFacing === 'up') {
        anim = SHEET.walkUp;
      } else {
        anim = SHEET.walkDown;
      }
      ...
      const [row, startCol, frameCount] = anim;
      const frameIdx = Math.floor(globalFrame / 2) % frameCount; // match agent animation speed
      const col = startCol + frameIdx;
      const sx = col * SHEET.frameW;
      const sy = row * SHEET.frameH;
      ...
      const knightFlip = knightFacing === 'right';
      ctx.save();
      if (knightFlip) {
        ctx.translate(px + offset + drawSize, 0);
        ctx.scale(-1, 1);
        ctx.drawImage(img, sx, sy, SHEET.frameW, SHEET.frameH,
          0, py + offset, drawSize, drawSize);
      } else {
        ctx.drawImage(img, sx, sy, SHEET.frameW, SHEET.frameH,
    ...
    if (obs.state === 'lurking' && obs.health < 1) {
      ...
      ctx.fillStyle = 'rgba(62, 46, 34, 0.6)';
  ...
  function trySpawnGato() {
    ...
    const dest = cozySpots.length > 0
      ? cozySpots[Math.floor(Math.random() * cozySpots.length)]
  ...
  function updateGato(gato, dt) {
    ...
    // Turn-pause for gatos (skip when fleeing — urgency overrides aesthetics)
    if (gato.turnPauseTimer > 0 && !gato.fleeing) {
      gato.turnPauseTimer -= dt;
      gato.state = 'idle';
      if (gato.turnPauseTimer <= 0) {
        gato.turnPauseTimer = 0;
        gato.state = 'walking';
      }
      return;
    }
    ...
    if (gato.path && gato.path.length > 0) {
      ...
      // Detect axis change and trigger turn pause (not when fleeing)
      if (!gato.fleeing && isAxisChange(gato.prevFacing, newFacing)) {
        gato.facing = newFacing;
        gato.prevFacing = newFacing;
        gato.turnPauseTimer = TURN_PAUSE_MS;
        gato.state = 'idle';
        return;
      }
      gato.facing = newFacing;
      ...
      // Clamp to walkable bounds
      gato.renderX = Math.max(WALK_MIN_X, Math.min(WALK_MAX_X, gato.renderX));
      gato.renderY = Math.max(WALK_MIN_Y, Math.min(WALK_MAX_Y, gato.renderY));
      return;
    }
  ...
  function drawGato(gato, timestamp) {
    ...
    if (img && img.complete && img.naturalWidth > 0) {
      ...
      if (gato.state === 'walking') {
        ...
        anim = gato.facing === 'up' ? SHEET.idleUp : SHEET.idleDown;
      }
      const [row, startCol, frameCount] = anim;
  ...
  function updateSpecialCreature(creature, dt) {
    // Turn-pause for special creatures
    if (creature.turnPauseTimer > 0) {
      creature.turnPauseTimer -= dt;
      creature.state = 'idle';
      if (creature.turnPauseTimer <= 0) {
        creature.turnPauseTimer = 0;
        creature.state = 'walking';
      }
      // Still check despawn during pause
    ...
    if (creature.path && creature.path.length > 0) {
      ...
      // Track facing direction from movement vector (consistent with agents/gatos)
      let newFacing;
      if (Math.abs(dx) > Math.abs(dy)) {
        newFacing = dx < 0 ? 'left' : 'right';
      } else {
      ...
      // Detect axis change and trigger turn pause
      if (isAxisChange(creature.prevFacing, newFacing)) {
        creature.facing = newFacing;
        creature.prevFacing = newFacing;
        creature.turnPauseTimer = TURN_PAUSE_MS;
        creature.state = 'idle';
        return;
      }
      creature.facing = newFacing;
      ...
      // Clamp to walkable bounds
      creature.renderX = Math.max(WALK_MIN_X, Math.min(WALK_MAX_X, creature.renderX));
      creature.renderY = Math.max(WALK_MIN_Y, Math.min(WALK_MAX_Y, creature.renderY));
  ...
  function drawSpecialCreature(creature, timestamp) {
    ...
    if (img && img.complete && img.naturalWidth > 0) {
      // Use persisted facing direction (set during updateSpecialCreature movement)
      const creatureFacing = creature.facing || 'down';

      let anim;
      if (creature.state === 'walking') {
        if (creatureFacing === 'up') {
          anim = SHEET.walkUp;
        } else {
          anim = SHEET.walkDown;
        }
      } else {
        anim = creatureFacing === 'up' ? SHEET.idleUp : SHEET.idleDown;
      }

      const [row, startCol, frameCount] = anim;
      const frameIdx = Math.floor(globalFrame / 2) % frameCount;
      const col = startCol + frameIdx;

      // Sprite sheet faces left by default — flip when facing right
      const creatureFlip = creatureFacing === 'right';
      ctx.save();
      if (creatureFlip) {
        ctx.translate(px + offset + size, 0);
        ctx.scale(-1, 1);
        ctx.drawImage(img, col * SHEET.frameW, row * SHEET.frameH, SHEET.frameW, SHEET.frameH,
          0, py + offset, size, size);
      } else {
        ctx.drawImage(img, col * SHEET.frameW, row * SHEET.frameH, SHEET.frameW, SHEET.frameH,
  ...
  return {
    ...
    CHAR_SPRITES, EXTRA_CHAR_SPRITES, SEASON_SPRITES,
    activeSpritePool: () => activeSpritePool,
    LABEL_COLORS,
  };
})();
