...
(function () {
  ...
  var ambientAgentIds = [];
  var savedScene = null;
  var lockedScene = null; // null = auto-rotate, string = locked scene name
   ...
   * When a scene is locked, the scene switch and audio scene change are
   * skipped — but narrative text and other events still fire normally.
   * @param {Object} act
   */
  function onActChange(act) {
    // Skip scene switch when locked to a specific scene
    if (!lockedScene) {
      // Switch scene
      if (typeof PixelAgents !== 'undefined' && PixelAgents.switchScene) {
  ...
  /**
   * Lock the fireplace to a specific scene — stops auto-rotation.
   * Immediately switches to that scene if fireplace is running.
   * @param {string} sceneName  One of the SCENE_POOL scene names.
   */
  function lockScene(sceneName) {
    // Validate the scene name exists in the pool
    var valid = false;
    for (var i = 0; i < SCENE_POOL.length; i++) {
      if (SCENE_POOL[i].name === sceneName) { valid = true; break; }
    }
    if (!valid) return;

    lockedScene = sceneName;

    // Immediately switch to the locked scene
    if (state === STATE_RUNNING) {
      if (typeof PixelAgents !== 'undefined' && PixelAgents.switchScene) {
        PixelAgents.switchScene(sceneName);
      }
      if (typeof PixelAudioEngine !== 'undefined' && PixelAudioEngine.setScene) {
        try { PixelAudioEngine.setScene(sceneName); } catch (_) { /* skip */ }
      }
    }

    document.dispatchEvent(new CustomEvent('fireplace:sceneLocked', {
      detail: { scene: sceneName },
    }));
  }

  /**
   * Unlock the scene — resume auto-rotation from the schedule.
   */
  function unlockScene() {
    lockedScene = null;

    // Re-sync to the current act's scene
    if (state === STATE_RUNNING) {
      var act = findCurrentAct(Date.now() - startTime);
      if (act) {
        if (typeof PixelAgents !== 'undefined' && PixelAgents.switchScene) {
          PixelAgents.switchScene(act.scene);
        }
        if (typeof PixelAudioEngine !== 'undefined' && PixelAudioEngine.setScene) {
          try { PixelAudioEngine.setScene(act.scene); } catch (_) { /* skip */ }
        }
      }
    }

    document.dispatchEvent(new CustomEvent('fireplace:sceneUnlocked'));
  }

  /**
   * @returns {string|null} The locked scene name, or null if auto-rotating.
   */
  function getLockedScene() {
    return lockedScene;
  }
