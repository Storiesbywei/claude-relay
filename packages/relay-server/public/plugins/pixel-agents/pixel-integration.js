...
window.PixelIntegration = (() => {
  ...
  const FireplaceUI = window.PixelFireplaceUI        || null;
  ...
  function togglePixelView() {
    ...
    if (isShowing) {
      ...
      // Stop polling intervals while hidden
      stopRefreshLoop();
      stopSpecialSpawnLoop();
  ...
  function wireFireplace() {
    ...
    btn.addEventListener('click', () => {
      ...
      if (Fireplace.isActive()) {
        ...
        Fireplace.start();
        // Start audio with a unique seed per session
        // Fireplace visual seed is deterministic (same show per 4hr window),
        // but audio should vary each time for freshness.
        if (Audio && !Audio.isPlaying()) {
          var audioSeed = (Fireplace.getSeed() ^ Date.now()) >>> 0;
          try { Audio.start(audioSeed); } catch (_) { /* skip */ }
        }
      }
    });
    ...
    document.addEventListener('fireplace:started', (e) => {
      const btn = $('#btn-fireplace');
      if (btn) {
        btn.innerHTML = PixelIcons.icon('square', 14) + ' Exit';
        btn.title = 'Exit fireplace mode (seed: ' + (e.detail && e.detail.seed) + ')';
      }
      // Activate fullscreen overlay
      if (FireplaceUI && FireplaceUI.activate) {
        FireplaceUI.activate();
      }
      refreshSidebar();
    });
    ...
    document.addEventListener('fireplace:stopped', () => {
      const btn = $('#btn-fireplace');
      if (btn) {
        btn.innerHTML = PixelIcons.icon('flame', 14) + ' Fireplace';
        btn.title = 'Ambient fireplace mode';
      }
      // Deactivate fullscreen overlay
      if (FireplaceUI && FireplaceUI.deactivate) {
        FireplaceUI.deactivate();
      }
      // Stop audio
      if (Audio && Audio.isPlaying()) {
    ...
    if (urlMode === 'pixel' || urlMode === 'fireplace') {
      ...
      requestAnimationFrame(() => {
        ...
        if (urlMode === 'fireplace' && Fireplace && !Fireplace.isActive()) {
          setTimeout(() => {
            // Use URL seed if provided (?mode=fireplace&seed=123248)
            var urlSeed = window.__fireplaceUrlSeed || null;
            Fireplace.start(urlSeed);
            if (Audio && !Audio.isPlaying()) {
              var audioSeed = (Fireplace.getSeed() ^ Date.now()) >>> 0;
              try { Audio.start(audioSeed); } catch (_) { /* skip */ }
            }
